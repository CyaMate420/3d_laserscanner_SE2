...
void setup() {                // evtl in Funktionen auslagern ? serial_init(), lcd_init() .....
  ...
  //serial
  Wire.setClock(400000);      // set I2C baud rate on max 400KHz
  Wire.begin();
  Serial.begin(115200);
...
void display_scan_progress(uint8_t current_value, uint8_t max_value){   // shows a progressbar in the bottom row and displays the percentage done 
  ...
  //static uint8_t st1 = (LCD_COLUMNS/2)-1;
  //static uint8_t st2 = (LCD_COLUMNS/2)+2;
  static uint8_t st1 = 6;
  static uint8_t st2 = 8;
  for(uint8_t n=0; n<progress_scaled; n++){
